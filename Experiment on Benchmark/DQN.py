...
class DQN(object):
    def __init__(
        ...
        self.r.seed(seed)
        # init network
        self.Q_net = Network2D(action_dim).to(device)
        self.Q_target = Network2D(action_dim).to(device)
        # init network parameters
        self.Q_net.apply(Network2D.init_weights)
    ...
    def choose_action(self, state, training: bool, testing: bool):
        q = self.get_choose_action_q_value(state)
#         print(q.shape)
        greedy_action = q.max(
                        -1).indices.item()
        greedy_action = 0
