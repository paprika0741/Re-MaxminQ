...
class MyEnv(object):
    ...
    def __init__(self, env_name  ,  device: TorchDevice) -> None:
    ...
    def evaluate(
        ...
        for _ in range(self.get_eval_lives() * num_episode):
            ...
            while not done:
                ...
                action = agent.choose_action(state, False,testing=True)
        ...
        self.__env = self.__env_train
        return np.sum(ep_rewards) / num_episode, frame
    

act_dic = {'Seaquest-v0': 18, 'Asterix-v0': 9, 'BreakoutNoFrameskip-v4': 4, 'SpaceInvaders-v0': 6,'SpaceInvadersNoFrameskip-v4':6}

class AtarEnv(object):
    ...
    def get_action_dim(self) -> int:
        """get_action_dim returns the reduced number of actions."""
        #不知道为什么 原来的breakout 实际的action dim - 1  
        return act_dic[self.env_name] - 1
