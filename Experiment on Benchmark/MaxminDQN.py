...
class MaxminDQN(DQN):
    def __init__(
        ...
        for i in range(self.network_number):
            # init network
            self.Q_net[i] = Network2D(action_dim).to(device)
            self.Q_target[i] = Network2D(action_dim).to(device)
            # init network parameters
            self.Q_net[i].apply(Network2D.init_weights)
            self.Q_target[i].load_state_dict(self.Q_net[i].state_dict())
            self.Q_target[i].eval()
            self.optimizer[i] = optim.RMSprop(
...
#         self.Q_net[0] = Network(action_dim).to(device)
#         self.Q_target[0] = Network(action_dim).to(device)
#         # init network parameters
#         self.Q_net[0].apply(Network.init_weights)
#         self.Q_target[0].load_state_dict(self.Q_net[0].state_dict())
#         self.Q_target[0].eval()
#         self.optimizer[0] = optim.Adam(
#                 self.Q_net[0].parameters(),
#                 lr=lr,
#                 eps=1.5e-4,
#             )
#         for i in range(1,self.network_number):
...
#             self.Q_target[i].load_state_dict(self.Q_net[0].state_dict())
#             self.Q_target[i].eval()
